...
This routine implements the dual method of Goldfarb and Idnani (1982, 1983) for solving quadratic programming problems of the form
  ...
  # Input parameter:
   * `dmat`   nxn matrix, the matrix D from above (typically Float64)
   ...
   * `Amat`   nxq matrix, the matrix A from above (typically Float64) [ A=(A1 A2)^T ], copied and thus unmodified.
...
function solveQP(dmat::AbstractMatrix{T}, dvec::AbstractArray{T},
    ...
    return solveQP!(Matrix(dmat), Vector(dvec), Amat, Vector(bvec); meq = meq, factorized = factorized)
end
...
This routine implements the dual method of Goldfarb and Idnani (1982, 1983) for solving quadratic programming problems of the form
   ...
   * `dvec`   nx1 vector, the vector d from above (typically Float64)
          contains on exit the solution to the initial, i.e.,
          unconstrained problem
   * `Amat`   nxq matrix, the matrix A from above (typically Float64) [ A=(A1 A2)^T ]
